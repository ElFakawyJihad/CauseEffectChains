...
/**
 * Classe permettant de naviguer entre les nodes pour transformer les informations qui nous intéressent
 * 
 * @author Quentin
 *
 */
public class NodeNavigator {
	...
	/**
	 * Permet d'itérer une list entière de Node pour la transformer et remplir la trace
	 * 
	 * @param nodes
	 * @return
	 * @throws EvalError
	...
	/**
	 * Permet de modifier un Node donné. On gère les expressions basiques et les boucles for
	 * 
	 * @param node
	 * @param index
	 * @param iterator
	...
	public static List<Node> transformNode(Node node, int index) throws EvalError {
		...
		List<Node> newNodes = new ArrayList<Node>(); //La node donnée + les nouvelles nodes
		...
		switch(cNode.getClass().getSimpleName()) {
			case "ExpressionStmt":
				...
				String varName = handleExpression(expressionStmt.getExpression()); //On récupère la variable concernée
				...
				newNodes.add((Node)t); //On ajoute la nouvelle Node, qui consiste à remplir la trace
				break;
			case "ForStmt":
				BlockStmt blockStmt = (BlockStmt) ((ForStmt)cNode).getBody(); //On récupère les instructions dans la boucle

				List<Node> tempNodes = transformNodes(blockStmt.getChildNodes()); //On altère chacune de ces nodes recursivement
	...
	/**
	 * Pour gérer une expression, qui est un cran en dessous de ExpressionStmt niveau hiérarchie.
	 * Permet surtout de récupérer le nom de la variable concernée.
	 * 
	 * @param e
	 * @return
	 */
